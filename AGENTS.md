...
숫자를 정하지 않는다. 1,000 → 4,000 을 거쳐 **지금은 상한이 없다** (#46).
검증은 빈 입력·공백만 두 겹으로 남아 있고, 그것으로 평가항목 `16` 을 충족한다.
...
| 누가 | 결과 | |
|---|---|---|
| **B** | 코디세이 OpenAI 호환 엔드포인트 | `config.py` 상수 · #35 #48 |
| **C** | Neon PostgreSQL (pooled) | 연결 문자열 DM 전달 완료 · #36 |

**연결 문자열과 API 키는 채팅방·이슈·커밋 어디에도 붙이지 않는다.** 공개 리포라
한 번 올라가면 재발급 외에 방법이 없다. 실제 값은 Vercel 대시보드 환경변수에만 둔다.
